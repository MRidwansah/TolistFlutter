...
class _TodolistScreenState extends State<TodolistScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _load() async {
    ...
    if (mounted) {
      setState(() {
        _todos = todos;
        _loading = false;
        _nextId = todos.isEmpty ? 100 : todos.map((t) => t.id).reduce((a, b) => a > b ? a : b) + 1;
      });
    }
  }
  ...
  Future<void> _addTask(String status, String text) async {
    ...
    final todo = Todo(id: _nextId++, task: text.trim(), status: status);
  ...
  Future<void> _moveTask(Todo todo, String newStatus) async {
    setState(() => todo.status = newStatus);
    await StorageService.saveTodos(_todos);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        // FIXED TabBar — pakai isScrollable:false + labelPadding kecil agar muat
        Container(
              ...
              return Tab(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    // FIXED: pakai FittedBox agar teks tidak overflow
                    Flexible(
                      child: Text(
                        labels[i],
                    ...
                    Container(
                      ...
                      child: Text('$count',
                          style: GoogleFonts.inter(color: colors[i], fontSize: 10, fontWeight: FontWeight.w900)),
                    ),
                  ],
                ),
              );
        ...
        child: widget.todos.isEmpty
            ? Center(
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Icon(Icons.inbox_outlined, color: AppColors.textSlate600, size: 38),
                  const SizedBox(height: 10),
                  Text('Tidak ada tugas',
                      style: GoogleFonts.inter(color: AppColors.textSlate600, fontSize: 13)),
                ]),
              )
            : ListView.builder(
        ...
        Padding(
          ...
          child: Column(children: List.generate(3, (_) => Padding(
            ...
            child: Container(width: 14, height: 2,
                decoration: BoxDecoration(color: AppColors.textSlate600, borderRadius: BorderRadius.circular(2))),
          ))),
        ),
        Expanded(
          child: Text(
