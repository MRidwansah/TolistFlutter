...
class _BookmarkedScreenState extends State<BookmarkedScreen> {
  ...
  Future<void> _load() async {
    ...
    if (mounted) {
      setState(() {
        _all = notes;
        _bookmarked = notes.where((n) => n.isBookmarked).toList();
        _loading = false;
      });
    }
  }
  ...
  Future<void> _removeBookmark(Note note) async {
    setState(() {
      ...
      _bookmarked.remove(note);
    });
    await StorageService.saveNotes(_all);
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return _BookmarkCard(
                            ...
                            onTap: () => Navigator.push(
                              context,
                              MaterialPageRoute(builder: (_) => NoteDetailScreen(note: note, allNotes: _all, onUpdated: _load)),
                            ),
                          );
                        },
                      ),
