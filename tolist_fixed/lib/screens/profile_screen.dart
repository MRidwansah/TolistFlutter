...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadData() async {
    ...
    final profileImg = await StorageService.getProfileImage();
    if (mounted) {
      setState(() {
        _notes = notes; // semua notes termasuk draft
        _loading = false;
        _profileImageBase64 = profileImg;
      });
    }
  }
  ...
  Future<void> _pickProfileImage() async {
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        String? b64;
        if (file.bytes != null) {
          b64 = base64Encode(file.bytes!);
        } else if (file.path != null) {
          b64 = base64Encode(await File(file.path!).readAsBytes());
        }
        if (b64 != null) {
          await StorageService.saveProfileImage(b64);
          if (mounted) setState(() => _profileImageBase64 = b64);
          _toast('Foto profil diperbarui!', true);
        }
      }
    } catch (e) {
      _toast('Gagal memilih foto', false);
    }
  }
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _sheetOption(
            ...
            label: 'Ganti Foto Profil', subtitle: 'Pilih gambar dari galeri',
  ...
  Future<bool> _confirm(String title, String content) async {
    return await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            actions: [
              TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text('Batal', style: TextStyle(color: AppColors.textSlate400))),
              ElevatedButton(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          const Spacer(),
          Icon(Icons.chevron_right, color: AppColors.textSlate600, size: 18),
        ]),
      ),
    );
  }
                        ...
                        child: Stack(children: [
                          Container(
                            ...
                            child: hasProfileImage
                                ? ClipRRect(borderRadius: BorderRadius.circular(22), child: Image.memory(base64Decode(_profileImageBase64!), fit: BoxFit.cover))
  ...
  // ─── Catatan Saya grid (hanya yang sudah dipublish) ───
  Widget _notesGrid() {
  ...
  // ─── Draft grid (catatan yang sengaja disimpan sebagai draft) ───
  Widget _draftGrid() {
  ...
  // Publish draft → ubah isDraft jadi false
  Future<void> _publishDraft(Note note) async {
    ...
    if (ok == true) {
      ...
      if (idx != -1) {
        setState(() => _notes[idx].isDraft = false);
        await StorageService.saveNotes(_notes);
...
class _NoteCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasImage = note.imageData != null && note.imageData!.isNotEmpty;
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          if (hasImage)
            ClipRRect(
              ...
              child: Image.memory(base64Decode(note.imageData!), width: double.infinity, height: 65, fit: BoxFit.cover),
            ),
          Expanded(
            child: Padding(
