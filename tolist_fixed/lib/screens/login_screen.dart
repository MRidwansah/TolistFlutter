...
// ─────────────────────────────────────────────
// Forgot Password Bottom Sheet (Supabase OTP)
// ─────────────────────────────────────────────
...
class _ForgotPasswordSheetState extends State<_ForgotPasswordSheet> {
  ...
  Future<void> _sendCode() async {
    ...
    try {
      ...
      setState(() { _error = 'Gagal mengirim email reset. Pastikan email terdaftar.'; });
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  }
  ...
  Future<void> _verifyCode() async {
    ...
    try {
      await supa.Supabase.instance.client.auth.verifyOTP(
      ...
      setState(() => _error = 'Kode verifikasi salah atau sudah kedaluwarsa.');
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  }
  ...
  Future<void> _resetPassword() async {
    ...
    try {
      await supa.Supabase.instance.client.auth.updateUser(
        supa.UserAttributes(password: newPass),
      );
  ...
  List<Widget> _stepEmail() => [
    ...
    const SizedBox(height: 20),
    _actionBtn('Kirim Kode Reset', Icons.send_rounded, _loading ? null : _sendCode),
  ];
  ...
  List<Widget> _stepCode() => [
    ...
    const SizedBox(height: 20),
    _actionBtn('Verifikasi Kode', Icons.verified_rounded, _loading ? null : _verifyCode),
