...
class _NoteDetailScreenState extends State<NoteDetailScreen> {
  ...
  Future<void> _deleteNote() async {
    ...
    if (confirm == true) {
      widget.allNotes.removeWhere((n) => n.id == _note.id);
      await StorageService.saveNotes(widget.allNotes);
  ...
  Future<void> _showEditDialog() async {
    ...
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: Text('Batal', style: TextStyle(color: AppColors.textSlate400))),
          ElevatedButton(
            onPressed: () async {
              final idx = widget.allNotes.indexWhere((n) => n.id == _note.id);
              if (idx != -1) {
                final updated = Note(
                  id: _note.id,
                  ...
                  imageData: _note.imageData,
                );
                widget.allNotes[idx] = updated;
                await StorageService.saveNotes(widget.allNotes);
  ...
  Widget build(BuildContext context) {
    ...
    final hasImage = _note.imageData != null && _note.imageData!.isNotEmpty;
